...
function calcDashboard(wb) {
  ...
  const hqRows  = XLSX.utils.sheet_to_json(wsHQ,  {defval:null});

  const N_WK = 9;
  const num = v => typeof v === 'number' ? v : null;
  const pct = v => typeof v === 'number' ? Math.round(v * 10000) / 100 : null;

  // Weekly arrays from rows 5-18
  ...
  let curLoc = null;
  hqRows.forEach(r => {
    if (r['สถานที่']) curLoc = r['สถานที่'];
    if (!curLoc) return;
    const qty = r['จำนวน\r\nTOR'] || r['จำนวน\nTOR'] || r['จำนวน TOR'] || 0;
    if (typeof qty !== 'number') return;
    if (!locMap[curLoc]) locMap[curLoc] = { tor: 0, mig: 0 };
    locMap[curLoc].tor += qty;
    const status = r['Status'] || r['Complete Date'] || '';
    if (status && status !== '-' && typeof status === 'string' && status.trim()) {
...
async function getDashboard(force = false) {
  ...
  cacheTime = now;
  console.log(`✓ Cached: mig=${cachedData.meta.mig}/${TOTAL} rate=${cachedData.insight.daily_rate}`);
  return cachedData;
}
